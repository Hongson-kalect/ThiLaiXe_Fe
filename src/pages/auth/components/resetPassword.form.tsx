...
export function ResetPassForm(props: ISignUpFormProps) {
  ...
  const getVerifyCode = async () => {
    await authUtil.resetPasswordMail(form.getFieldValue("email"));
  };
  ...
  const onVerify = async () => {
    ...
    if (data?.status === 200) {
      const data = await authUtil.resetPassword({
        email: form.getFieldValue("email"),
  ...
  return (
    <div className="form-wrapper">
      <CustomTitle className="title">Đặt lại mật khẩu</CustomTitle>
      <Form
        form={form}
        className="form"
        labelAlign="left"
